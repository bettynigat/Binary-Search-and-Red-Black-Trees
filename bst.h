...
class IntBinaryTree {
 public:
	...
	void setRoot(IntEntry *r) {root = r;}
...
class IntBST {
 public:
	IntBST() {cnt = 0; range=0; }
	int size() const {return cnt;}
	bool empty() const { return (size() == 0);}
	virtual int find(int k);
	virtual IntEntry* findRecursive(IntEntry* p, int k);
	virtual IntEntry* insertRecursive(IntEntry* p, int k); 

	void inorderRecursive(IntEntry* p);
	void inorder();

	virtual void insert(int k);
	virtual bool remove(int k);
	IntEntry* removeRecursive(IntEntry* p, int key);

	int successorRecursive(IntEntry* p);
	int successor(int k);
	

	int min();
	int minRecursive (IntEntry* p);
	void print();

	void setRange(int m){range=m;}
	int getRange() {return range;}
	int countRange(int k1, int k2);
	void countRangeRecursive(IntEntry* p, int k1, int k2, int& range);
...
class IntRBTree : public IntBST {
 public:
	virtual void insert(int k);
	virtual bool remove(int k);
	IntEntry* restructureRL(IntEntry* p);
	IntEntry* restructureRR(IntEntry* p);
	IntEntry* restructureLL(IntEntry* p);
	IntEntry* restructureLR(IntEntry* p);
	void doubleBlackRemedy(IntEntry* r);
	IntEntry* deleted_node(IntEntry* root,int key);


	void remedyDoubleRed(IntEntry*root, IntEntry*p); 
